...
export class PresenceService {
    ...
    private static toSnapshot(client: Client): PresenceSnapshot | null {
        ...
        const roomId = Number.isFinite(client.currentRoomId) && client.currentRoomId >= 0 ? client.currentRoomId : null;
        const roomLabel = roomId === null ? null : `Room ${roomId + 1}`;
        ...
        const stateParts = [characterName];

        if (party.partySize > 1) {
            stateParts.push(`Party ${party.partySize}/${PARTY_MAX_MEMBERS}`);
        }
        if (roomLabel) {
            stateParts.push(roomLabel);
        }

        return {
            characterName,
            characterClass: String(client.character?.class ?? '').trim(),
            levelKey,
            levelName,
            activityKind,
    ...
    private static getPartySnapshot(characterName: string): PartySnapshot {
        ...
        if (!characterKey) {
            return { partyId: null, partySize: 1, partyLeader: null };
        }
